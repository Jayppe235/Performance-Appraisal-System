...
import { AlertTriangle, CheckCircle2, ClipboardList, GraduationCap, Search, Target, Users } from 'lucide-react';
...
export default function ProgramHeadDashboard({ role }) {
  ...
  const { metrics: liveMetrics, actionCenter: apiActionCenter, loading, error, timestamp } = useRealtimeMetrics('program_head', {
  ...
  return (
    <section className="admin-content admin-module dean-content">
      {activeSection === 'overview' && (
        <>
          ...
          <section className="admin-dashboard-unified module-wide page-enter" aria-labelledby="program-head-dashboard-unified-title">
            <div className="action-center-head">
              <div>
                <p className="eyebrow">Dashboard</p>
                <h2 id="program-head-dashboard-unified-title">Dashboard Summary</h2>
                <p>{error ? `Live refresh paused: ${error}` : `Review current program appraisal status${timestamp ? `, updated ${new Date(timestamp * 1000).toLocaleTimeString()}` : ''}.`}</p>
              </div>
