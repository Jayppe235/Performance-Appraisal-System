...
function PerformanceReportWorkspace({ role }) {
  ...
  function resetFilters() {
    ...
    const scopedProgram = isProgramHead && metadata.programs[0] ? metadata.programs[0].code : '';
  ...
  const showDepartmentColumn = report?.report_type !== 'department';
  const displayScore = (score) => score === null || score === undefined ? 'N/A' : Number(score).toFixed(2);
  const periodLabel = report?.period ? [report.period.school_year || report.period.period_name, report.period.semester].filter(Boolean).join(' / ') : 'All Evaluation Periods';
  const selectedProgram = metadata.programs.find((item) => item.code === filters.program) || metadata.programs[0];
  ...
  const selectedPeriodLabel = selectedPeriod ? `SY ${selectedPeriod.school_year || selectedPeriod.period_name}${selectedPeriod.semester ? ` • ${selectedPeriod.semester}` : ''}` : 'No period selected';
  ...
  const statusScopeLabel = isProgramHead ? filters.program : selectedDepartment?.name || (isDean ? scopedDepartment?.name : 'All Departments');
  ...
  return <section className={`performance-report-workspace program-report-workspace module-wide page-enter report-workspace-${roleKey}`}>
    ...
    <section className="program-report-scope" aria-labelledby="reporting-scope-title"><div className="program-report-card-heading"><div><LockKeyhole size={18} /><div><h3 id="reporting-scope-title">Your Reporting Scope</h3><p>{reportingScopeDescription}</p></div></div><span><LockKeyhole size={13} /> {isDean || isProgramHead ? 'Assigned scope' : 'Authorized access'}</span></div><div className="program-report-scope-grid">
      ...
      <article><GraduationCap size={19} /><div><span>Program Scope</span>{isProgramHead ? <strong tabIndex="0" title={selectedProgram ? `${selectedProgram.code} — ${selectedProgram.name}` : 'Assigned program'}><b>{selectedProgram?.code || filters.program || 'Program'}</b><small>{selectedProgram?.name || 'Assigned program'}</small></strong> : <strong>{isDean ? 'Programs within assigned department' : 'All Academic Programs'}</strong>}</div></article>
    ...
    <section className="performance-report-config"><div className="performance-report-section-head"><SlidersHorizontal size={19} /><div><h3>Report Configuration</h3><p>Choose report options, preview the output, then export when ready.</p></div></div><div className={`performance-report-filter-grid ${isProgramHead ? 'program-report-filter-grid' : 'leader-report-filter-grid'}`}>
      ...
      {!isProgramHead && <label>Department<select value={filters.department_id} disabled={isDean || filters.report_type === 'overall_department'} onChange={(e) => updateFilter('department_id', e.target.value)}><option value="">{isDean ? 'Assigned Department' : 'All Departments'}</option>{metadata.departments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>}
      ...
      <label>Evaluation Period<select id="program-report-period" aria-describedby={isProgramHead ? 'program-period-help' : undefined} value={filters.period_id} onChange={(e) => updateFilter('period_id', e.target.value)}><option value="">Select Evaluation Period</option>{metadata.periods.map((item) => <option key={item.id} value={item.id}>SY {item.school_year || item.period_name}{item.semester ? ` • ${item.semester}` : ''}</option>)}</select>{isProgramHead && <small id="program-period-help">Uses the selected appraisal period for this report.</small>}</label>
    ...
    {report && <section className="performance-report-preview" id="performance-report-preview"><div className="performance-report-paper" data-report-ready="true">
      <header><div className="performance-report-brand"><span className="performance-report-logo"><img src={assetUrl(report.institution_logo || 'assets/images/ndmc-seal.png')} alt="Notre Dame of Midsayap College seal" onError={(event) => { const image = event.currentTarget; const fallback = assetUrl('assets/images/ndmc-seal.png'); image.onerror = () => { if (image.parentElement) image.parentElement.style.display = 'none'; }; if (image.src !== new URL(fallback, window.location.origin).href) image.src = fallback; else if (image.parentElement) image.parentElement.style.display = 'none'; }} /></span><div><h2>Notre Dame of Midsayap College</h2><h3>{overall ? 'Institutional Academic Departments' : report.department}</h3><p>Midsayap, Cotabato</p></div>{report.department_logo && <span className="performance-report-logo department"><img src={assetUrl(report.department_logo)} alt={`${report.department} logo`} onError={(event) => { if (event.currentTarget.parentElement) event.currentTarget.parentElement.style.display = 'none'; }} /></span>}</div><hr /><div className="performance-report-title"><h4>{overall ? 'OVERALL DEPARTMENT PERFORMANCE REPORT' : 'PERFORMANCE FACTORS APPRAISAL'}</h4><strong>{report.role_label}</strong><span>{report.period?.school_year ? `SY ${report.period.school_year}` : report.period?.period_name || 'All Evaluation Periods'}</span>{report.period?.semester && <span>{report.period.semester}</span>}</div></header>
      ...
      <div className="performance-report-table-wrap"><table className={showDepartmentColumn ? 'with-department' : 'single-department'}><thead><tr>{overall ? <><th>Department</th><th>Personnel</th><th>Peer Mean</th><th>Head Mean</th><th>PH/SC Mean</th><th>Overall Mean</th><th>Level of Performance</th></> : <><th>Name</th>{showDepartmentColumn && <th>Department</th>}<th>Program</th><th>Peer</th><th>Head</th><th>PH/SC</th><th>Total</th><th>Mean</th><th>Level of Performance</th></>}</tr></thead><tbody>{report.rows.map((row) => overall ? <tr key={row.department}><td>{row.department}</td><td>{row.personnel}</td><td>{displayScore(row.peer)}</td><td>{displayScore(row.head)}</td><td>{displayScore(row.phsc)}</td><td>{displayScore(row.mean)}</td><td>{row.level}</td></tr> : <tr key={row.id}><td>{row.name}</td>{showDepartmentColumn && <td>{row.department}</td>}<td>{row.program || 'N/A'}</td><td>{displayScore(row.peer)}</td><td>{displayScore(row.head)}</td><td>{displayScore(row.phsc)}</td><td>{displayScore(row.total)}</td><td>{displayScore(row.mean)}</td><td><span className={`performance-level ${row.level.toLowerCase().replaceAll(' ','-')}`}>{row.level}</span></td></tr>)}{report.rows.length === 0 && <tr><td colSpan={overall ? 7 : showDepartmentColumn ? 9 : 8} className="empty">No completed evaluation results found for the selected filters.</td></tr>}</tbody></table></div>
      <section className="performance-report-legend" aria-label="Report legend"><strong>Legend</strong><div><span className="excellent"><i />Excellent <b>4.50–5.00</b></span><span className="very-satisfactory"><i />Very Satisfactory <b>3.75–4.49</b></span><span className="satisfactory"><i />Satisfactory <b>3.00–3.74</b></span><span className="needs-improvement"><i />Needs Improvement <b>Below 3.00</b></span><span className="incomplete"><i />Incomplete / N/A <b>Required results unavailable</b></span></div><small>Peer = peer evaluators · Head = Dean or Program Head · PH/SC = Program Head / Section Chair · Mean = overall computed rating</small></section>
