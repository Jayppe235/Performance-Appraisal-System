...
try {
    ...
    foreach ($facultyRows as $faculty) {
        if (!in_array(dean_summary_api_resolved_role($faculty), ['teacher', 'program_head'], true)) {
            continue;
        }
    ...
    if ($facultyIds !== []) {
        ...
        foreach ($allResults as $row) {
            ...
            $programCategoryBuckets[$programCode][$cat] ??= ['total' => 0.0, 'count' => 0, 'faculty' => []];
            $programCategoryBuckets[$programCode][$cat]['total'] += $avg;
            $programCategoryBuckets[$programCode][$cat]['count']++;
            $programCategoryBuckets[$programCode][$cat]['faculty'][$fid] = true;
    ...
    if ($facultyIds !== []) {
        $facultyPlaceholders = implode(',', array_fill(0, count($facultyIds), '?'));
        $planRows = admin_all(
            "SELECT p.*, f.full_name AS faculty_name, COALESCE(NULLIF(f.program_code, ''), 'Unassigned Program') AS program_code
    ...
    // Always derive department, program, and faculty recommendations from overall submitted results.
    $generatedPlans = [];
    if ($allFacultyComplete && $factorSummary !== []) {
        $departmentWeak = (string) ($factorSummary[0]['weakArea'] ?? $factorSummary[0]['factor'] ?? 'Professional Growth');
        $lastFactor = end($factorSummary);
        $departmentStrong = is_array($lastFactor) ? (string) ($lastFactor['weakArea'] ?? $lastFactor['factor'] ?? '') : '';
        $generatedPlans[] = [
            'id' => 0,
            'scope' => 'Department',
            'facultyName' => '',
            'program' => 'All Programs',
            'weakArea' => $departmentWeak,
            'facultyCount' => count($facultyById),
            'seminar' => dean_summary_api_seminar($departmentWeak),
            'recommendation' => dean_summary_api_action($departmentWeak, 'the department') . ($departmentStrong !== '' ? ' Maintain strengths in ' . $departmentStrong . '.' : ''),
            'status' => 'Identified',
        ];
    }
    ...
    foreach ($facultyById as $facultyId => $faculty) {
        ...
        $facultyByProgram[$prog][] = $facultyId;
    }

    $programSummaries = [];
    $facultyRecommendations = [];
    if ($allFacultyComplete) {
        foreach ($facultyByProgram as $programCode => $fIds) {
            $programWeakCounts = [];
            $programFields = [];

            foreach (($programCategoryBuckets[$programCode] ?? []) as $cat => $bucket) {
    ...
    foreach ($trainingPlans as $plan) {
        $trainingPlanKeys[strtolower(($plan['scope'] ?? '') . '|' . ($plan['facultyName'] ?? '') . '|' . ($plan['program'] ?? '') . '|' . ($plan['weakArea'] ?? ''))] = true;
    }
    if ($allFacultyComplete) {
        foreach ($generatedPlans as $plan) {
            $key = strtolower(($plan['scope'] ?? '') . '|' . ($plan['facultyName'] ?? '') . '|' . ($plan['program'] ?? '') . '|' . ($plan['weakArea'] ?? ''));
    ...
    // --- Weak Area Register — only show when ALL faculty evaluations are complete ---
    $weakAreas = [];
